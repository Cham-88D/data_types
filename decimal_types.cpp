...
/**
 * This shows float and double data type  initialization methods , Precision and scientific notation
 */
...
int main() {
    ...
    /*
     * we have to use 'f' after  floating number
     * unless compiler has to  automatically convert double to float before assigning
     * same for the long double 'L'
     */
    ...
    //Usually  mingw allows 6 precision points to print
    ...
    //after exceeding the precision digit limit garbage values are printed


    /* Float problems : The precision is usually too limited
     * 7 digits is not enough for most applications
     */

    float number4 = 192400023.123120f;               // Error : narrowing conversion
    ...
    //+infinity
    double result2 { number14 / number11 };
    std::cout << number14 << "/" << number11 << "  yields " << result2 << std::endl;
    std::cout << result2 << " + " << number14 << " yields " << result2 + number14 << std::endl;
